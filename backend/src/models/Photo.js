class Photo {
        ...
        static async insertPhoto(db, id, title, caption, imageSrc, placeholderSrc) {
                const sql = `INSERT INTO ${this.tableName} (id, title, caption, image_src, placeholder_src)
			 VALUES ($1, $2, $3, $4, $5, $6) RETURNING *`;
                const result = await db.query(sql, [id, title && title.trim() ? title : null, caption && caption.trim() ? caption : null, imageSrc, placeholderSrc]);
                return result.rows[0] ? result.rows[0] : null;
        }
	...
	static async editPhoto(db, id, title, caption) {
		const sql = `UPDATE ${this.tableName} SET title = $1, caption = $2 WHERE id = $3 RETURNING *`
